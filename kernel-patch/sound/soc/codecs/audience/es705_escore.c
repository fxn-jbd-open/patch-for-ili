...
static DEVICE_ATTR(power_state, 0444, es705_power_state_show, NULL);
...
static DEVICE_ATTR(pm_enable, 0666, es705_get_pm_enable, es705_set_pm_enable);
...
static DEVICE_ATTR(escore_debug, 0444, escore_debug_show, NULL);
...
static DEVICE_ATTR(profile_enable, 0666,
	...
	while (next_power_state != escore->escore_power_state) {
		...
		case ES_SET_POWER_STATE_NORMAL:
			...
			if (escore->escore_power_state ==
				...
				if (rc < 0) {
					pr_err("%s() Event response fail: %d\n",
							__func__, rc);
					goto power_transition_exit;
				}
		...
		case ES_SET_POWER_STATE_VS_LOWPWR:
			/* switch from vs low power to ns */
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	rc = escore_cmd(escore, (ES_SET_SYNC_WORDS << 16 |
			...
			&cmd_resp);
	if (rc) {
		dev_err(escore_priv.dev, "%s(): Set Sync words fail %d\n",
			__func__, rc);
		goto cmd_error;
	}
...
int es705_core_probe(struct device *dev)
{
	...
	const char *fw_fn_1st_stg = "audience/es814/audience-es814-fw1st.bin";
#if defined(CONFIG_SND_SOC_ES814_COMPRESSED_FW)
	const char *fw_filename = "audience/es814/BoskoUnzipApp.bin";
#else
	const char *fw_filename = "audience/es814/audience-es814-fw_4m.bin";
#endif
#else
	const char *fw_filename = "audience/es814/audience-es814-fw.bin";
#endif
#ifdef CONFIG_SND_SOC_ES_VS	/*fxn, start*/
	const char *vs_filename = "audience/es814/audience-es814-vs.bin";
	const char *bkg_filename = \
		"audience/cvqmodels/adnc_cvq_detection_bkg_w_hdrs.bin";
#endif /*fxn, end */
	const char *fw_af_filename = "audience/es814/audience-es814-af.bin";
#else
