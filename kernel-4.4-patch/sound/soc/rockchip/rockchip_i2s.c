...
/*fxn, add{*/
	/*in order to meet tfa9890 power-off timing requirement*/
	/*use this work to turn off i2s after tfa9890 i2c is done*/
	struct delayed_work i2s_off_work;
	struct workqueue_struct *i2s_wq;
/*fxn,end}*/
};
...
/*fxn, add{*/
static void i2s_off_task(struct work_struct *work)
{
	struct rk_i2s_dev *i2s = container_of(work, struct rk_i2s_dev, i2s_off_work.work);
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		/*fxn, modified{*/
		/*i2s ws signal shall be left till tfa9890 i2c is done*/
		/*therefore, using a delay work to turn off i2s tx*/
		else {
			queue_delayed_work(i2s->i2s_wq,
					&i2s->i2s_off_work,
					msecs_to_jiffies(200));
			//rockchip_snd_txctrl(i2s, 0); // move to work to turn off i2s
		}
		/*fxn, end}*/
		break;
	default:
...
/*fxn, add{*/
	i2s->i2s_wq = create_singlethread_workqueue("rockchip_i2s_work");
	INIT_DELAYED_WORK(&i2s->i2s_off_work, i2s_off_task);
/*fxn, end}*/
	return 0;
...
/*fxn, add{*/
	cancel_delayed_work_sync(&i2s->i2s_off_work);
	if (i2s->i2s_wq)
		destroy_workqueue(i2s->i2s_wq);
/*fxn, end}*/
