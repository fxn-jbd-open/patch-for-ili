...
static int escore_uart_boot_sequence(struct escore_priv *escore);
...
int escore_uart_boot_sequence(struct escore_priv *escore)
{
	 ...
	 * is enabled.
	 */
	if ((!escore->enable_4m_baud) && (escore->pdata->enable_hs_uart_intf)) {
	...
	if (escore->enable_4m_baud == false)
		msleep(20);
...
int escore_uart_boot_setup(struct escore_priv *escore)
{
	...
	rc = escore_uart_boot_sequence(escore);
	if (rc)
		return rc;

#if defined(CONFIG_SND_SOC_ES814_4M_BAUD_UART)
	rc = escore->bus.ops.high_bw_write(escore,
			(char *)escore->stnd1st->data,
			escore->stnd1st->size);
	if (rc) {
		dev_err(escore->dev, "%s(): firmware 1st stage write error %d\n",
				__func__, rc);
		rc = -EIO;
		return rc;
	}
	...
	escore_default_uart_baud = CONFIG_SND_SOC_ES_UARTHS_BAUD;
	/* set speed to Bosko APP baud */
	escore_configure_tty(escore_uart.tty, escore_default_uart_baud,
			UART_TTY_STOP_BITS);
...
int escore_uart_boot_finish(struct escore_priv *escore)
{
	...
	/* If gpio-a is defined and baud rate is 4M
	 * change it to 3M
	 */
...
#else
	if (escore->enable_4m_baud == true)
		escore_uart.baudrate_sbl = UART_TTY_BAUD_RATE_115_2_K;
	else
